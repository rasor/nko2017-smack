...
export interface IFacebookAPIMusicSong {
 id: string;
 url: string;
 type: string;
 title: string;
...
export interface IFacebookUserAuthenticate {
...
export interface IFacebookUIParameters {
 method?: string;
 name?: string;
 link?: string;
 picture?: string;
...
export interface IFacebook {
 ...
 logout(cb?: (response?: Object) => any ): any;
 ...
 Event: {
   ...
   */
  subscribe ( name: string, cb: (response: IFacebookUserAuthenticate) => any ): any;

  /**
   * name: 'auth.logout'
   */
  subscribe ( name: string, cb: (response: { status: string; }) => any ): any;

  /**
   * name: 'xfbml.render'
   */
  subscribe ( name: string, cb: () => any ): any;
   ...
   * name: 'message.send'
   */
  subscribe ( name: string, cb: (response: string ) => any ): any;

  /**
   * name: 'comment.remove'
   */
  subscribe(name: string, cb: (response: { href: string; commendID: string; } ) => any ): any;

  /**
   * @see http://developers.facebook.com/docs/reference/javascript/FB.Event.unsubscribe/
   */
  unsubscribe ( name: string, cb: (response: string ) => any ): any;
 };

 XFBML: {
  parse(dom?: Element, cb?: () => any): any;
 };
